...
for i=1:length(coords)
    ...
    %disp(coords(i,2));
		...
		if (or(((coords(i,1)== 1) && (coords (i,2)== 32)) , ((coords(i,1)== 64 && coords (i,2)== 32)),
    ((coords(j,1)== 1) && (coords (j,2)== 32)),((coords(j,1)== 64 && coords (j,2)== 32))))
    %eliminate two transmitters link
    if((coords(i,1)== 1) && (coords (i,2)== 32)&& (coords(j,1)== 64) && (coords (j,2)== 32))
    continue
    endif
		%if (a[1,2]==[1,32])
    %if (eq(coords(i,1),1) && (coords (i,2)== 32)) 
    %link points
